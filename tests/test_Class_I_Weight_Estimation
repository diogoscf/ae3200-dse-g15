...
def test_PowertrainWeight():
    ...
    assert math.isclose(class_I.PowertrainWeight(p_bat), (1-p_bat)*(MTOW/WP)/(eta_powertrain*p_ptr), rel_tol=1e-3)
...
def test_BatteryWeight():
    ...
    assert math.isclose(class_I.BatteryWeight(p_bat), MTOW / WP * E * p_bat/(e_bat * eta_bat * eta_electricmotor * DOD_bat), rel_tol=1e-3)


def test_FuelWeight():
    class_I = WeightEstm(aircraft_data)

    assert math.isclose(class_I.FuelWeight(class_I.dict["Power_prop"]["bat"]), 
                        1.15*(1-class_I.dict["Power_prop"]["bat"]) * 9.81 * class_I.dict["Iterations Class I"]["MTOW_kg"] / class_I.dict["Performance"]["W/P_N/W"] *\
                            class_I.dict["Performance"]["endurance"] / (class_I.dict["Power_prop"]["eta_generator"] * class_I.dict["Power_prop"]["E_fuel_Wh/kg"]), rel_tol=1e-3)
...
def test_WingWeight():
    ...
    assert math.isclose(class_I.WingWeight(), 
                        class_I.dict["Iterations Class I"]["Aw"] * class_I.dict["Iterations Class I"]["MTOW_kg"] + class_I.dict["Iterations Class I"]["Bw"],
                        rel_tol=1e-3)
...
def test_Iterations():
    ...
    while np.abs((MTOW_new - class_I.dict["Iterations Class I"]["MTOW_kg"]) / class_I.dict["Iterations Class I"]["MTOW_kg"]) > 0.02:
        if ok: 
        ...
        MTOW_new = OEW_prime + PowertrainWeight + BatteryWeight + FuelWeight + WingWeight + class_I.dict["Iterations Class I"]["Wpl_des_kg"]
    ...
    if MTOW_new < 4000:
        ...
        assert np.allclose(class_I.Iterations(class_I.dict["Power_prop"]["bat"]), 
                                            (MTOW_new,
                                        ...
                                        class_I.dict["Contingency"] * class_I.dict["Iterations Class I"]["Wpl_des_kg"]), rtol=1e-3)
