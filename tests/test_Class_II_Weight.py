...
aircraft_data = {
        ...
        "Performance": {"n_ult": 3, "VH_m/s": 5, "Vc_m/s": 6, "N_pax": 4, "M_D": 0.8},
        ...
        "General": {"Paint": True, "PoweredFlightControls": True, "DuplicatedFlightControls": True, "N_pax": 3, "N_row": 3, "APU": False}
    }
...
def test_landing_gear_weight(aircraft_data = aircraft_data):
    ...
    result1 = 0.013 * weight_class.W_TO + 0.362 * weight_class.W_L ** 0.417 * weight_class.n_ult_l ** 0.95 * weight_class.l_s_m ** 0.183 + 6.2 + 0.0013 * weight_class.W_TO + 0.007157 * weight_class.W_L ** 0.749 * weight_class.n_ult_l * weight_class.l_s_n ** 0.788
...
def test_structure_weight_total():
    ...
    assert math.isclose(weight_class.StructureWeight_Total(), weight_class.WingWeight()['Average'] + weight_class.EmpennageWeight()['Average'] + weight_class.FuselageWeight()['Average'] + weight_class.NacelleWeight()['Average'] + weight_class.LandingGearWeight()['Average'], rel_tol=1e-3)
...
def test_fuel_system_weight():
    ...
    result2 = 2.49 * ((weight_class.W_F/ weight_class.K_fsp) ** 0.6 * (1 / (1 + weight_class.int)) ** 0.3 * weight_class.N_t ** 0.2 * weight_class.N_e ** 0.13) ** 1.21
    result3 = 2 * (weight_class.W_F / 5.87) ** 0.667 #TODO: I have no idea if we have single piston or multi piston in the end so please verify this(pg.91)
...
def test_hydraulics_pneumatics():
    ...
    assert math.isclose(weight_class.HydraulicsPneumatics()['Average'], (0.006 + 0.0120) / 2 * weight_class.W_TO , rel_tol=1e-3)
...
def test_electrical_system_weight():
    ...
    result2 = 426 * ((weight_class.InstrumentsAvionicsElectronics()['Average'] + weight_class.FuelSystemWeight()['Average'])/1000) ** 0.51
...
def test_aircon_pressurization_anti_decing_weight():
    ...
    result1 = 0.265 * weight_class.W_TO ** 0.52 * weight_class.N_pax ** 0.68 * weight_class.InstrumentsAvionicsElectronics()['Average'] ** 0.17 * weight_class.M_D ** 0.08
    ...
    assert math.isclose(weight_class.AirconPressurizationAntiDeicingWeight()['Average'], (result1 + result2) / 2, rel_tol=1e-3)
...
def test_fixed_equipment_weight_total():
    ...
    assert math.isclose(weight_class.FixedEquipmentWeight_Total(), (weight_class.FlightControlSystem()['Average'] + weight_class.HydraulicsPneumatics()['Average'] + weight_class.InstrumentsAvionicsElectronics()['Average'] + weight_class.ElectricalSystemWeight()['Average'] + weight_class.AirconPressurizationAntiDeicingWeight()['Average'] + weight_class.OxygenSystem()['Average'] + weight_class.APU_Weight()['Average'] + weight_class.Furnishings()['Average'] + weight_class.AuxiliaryGear()['Average'] + weight_class.Paint()['Average']), rel_tol=1e-3)

def test_new_empty_weight():
    # Initialize the Class_II_Weight object
    weight_class = Class_II_Weight(aircraft_data)

    assert math.isclose(lbs_to_N(weight_class.NewEmptyWeight()), lbs_to_N(weight_class.PowerplantWeight_Total()['Average'] + weight_class.StructureWeight_Total() + weight_class.FixedEquipmentWeight_Total()), rel_tol=1)

def test_NewOEW():
    # Initialize the Class_II_Weight object
    weight_class = Class_II_Weight(aircraft_data)

    assert math.isclose(weight_class.NewOEW(), weight_class.NewEmptyWeight() + aircraft_data["Weights"]["W_Pilot_N"], rel_tol=1)