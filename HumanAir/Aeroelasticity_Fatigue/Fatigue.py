    ...
    if not verification:
        if not Experimental_SN:
            ...
            if K_t == None:
                ...
                if Smax != None:
                    i = (np.log(Smax) - np.log(Sult)) / ((np.log(alpha * Sult) - np.log(Sult))/(np.log(N_max) - np.log(N_min))) + (np.log(N_min))
                ...
                if Smax != None:
                    ...
                    plt.text(np.exp(i), Smax, '({:.2e}, {:.2e})'.format(np.exp(i), Smax), fontsize=12, verticalalignment='bottom')
            ...
            if Smax != None:
                for i in range(len(data)):
                    if Smax <= data[i,1] and Smax >= data[i+1,1]:
                        Nf = int(np.exp(np.log(data[i,0]) + (np.log(data[i+1,0]) - np.log(data[i,0]))/(np.log(data[i+1,1]) - np.log(data[i,1])) * np.log((Smax / data[i,1]))))
                        break         
...
if __name__ == '__main__':
    fatigue_life(Sult = 500, alpha = 0.1, Smax = 219, verification = False, Experimental_SN = False)
    fatigue_life(Sult = 500, alpha = 0.1, Smax = 219, verification = False, Experimental_SN = True)