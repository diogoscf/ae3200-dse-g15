...
class WP_WS:
    ...
    def __init__(self):
        ...
        self.ReferenceWS=[449.0307667, 602.1288757, 643.1831172, 725.2916003, 725.2916003, 578.0625962,
        ...
        self.ReferenceWP=[0.089508018, 0.082049016, 0.080130987, 0.068752535, 0.068752535, 0.058831561,
...
#       ========== 1: Calculate Values ==========
        self.Stallspeed_x=eq.Stallspeedx(0, 0, p.V_stall, p.Clmax_Land) #Choose if we want to use Cl with clean config or landing config
        ...
        self.Cruise_y=eq.Cruise(p.eta_p, p.h_Cruise, p.temp_offset, p.Cdo, p.V_cruise, self.WS, p.A, p.e, p.CruisePower, p.CruiseWeight)
        self.Climbrate_y=eq.Climbrate(p.eta_p, p.A, p.e, 0, 0, p.Cdo, p.climbrate, self.WS)
        self.Climbgradient_y=eq.Climbgradient(p.eta_p, self.WS, p.climbgradient, p.V_climb, p.A, p.e, p.Clmax_clean, p.Cl_SafetyFactor, p.Cdo, 0, 0)
