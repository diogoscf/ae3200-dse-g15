...
def Landingx(Clmax_land, h, temp_offset, sland, f, T0=288.15, constlambda=-0.0065):
    return (Clmax_land*Density(h, temp_offset)*sland/0.305199384478051392)/(2*f) # was 0.5915 changed to 0.305199384478051392

def Cruise(etap, h, temp_offset, Cd0, Vcruise, WS, A, e, cruisepowersetting, cruiseweightwrtMTOW, rho0=1.225, T0=288.15, constlambda=-0.0065):
    a=(cruisepowersetting/cruiseweightwrtMTOW)*etap*(Density(h, temp_offset)/rho0)**0.75
    b=((Cd0*0.5*Density(h, temp_offset)*Vcruise**3)/(cruiseweightwrtMTOW*WS)+(cruiseweightwrtMTOW*WS)/(np.pi*A*e*0.5*Density(h, temp_offset)*Vcruise))**(-1)
    return a*b
