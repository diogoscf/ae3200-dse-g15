...
if __name__ == "__main__":
    ...
    for i, row in flight_data.iterrows():
        ...
        print(i, dist.mean(), dist.median(), row["distance_before_refuelling_min"], row["distance_before_refuelling_max"])
        simulated_lengths = dist.rvs(size=10000)
        plt.plot(x, dist.pdf(x), colours[i] + "-", lw=2, label=i)
    ...
    if input("Pickle distribution? (y/N): ").lower() == "y":
        with open(os.path.join(os.path.dirname(__file__), "flightdist.pickle"), "wb+") as f:
            pickle.dump(overall_dist, f)