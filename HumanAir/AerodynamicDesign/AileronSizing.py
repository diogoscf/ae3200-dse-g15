...
def AileronDerivatives(acd=aircraft_data): # sizing method starting from p 466 Roskam Part VI
    ...
    c_doubleprime_h_alpha = c_prime_h_alpha + 2 * cl_alpha_theory * (1 - cl_alpha_cl_alpha_theory) * (np.tan(PHI_doubleprime_te/2) - 0.137) #NOT SURE IF IT IS MAX or LOCAL t/c

    balance_ratio = np.sqrt((0.05/(acd["Aileron"]["hinge_position"] - 0.05))**2 - (6.31 / 2 / (acd["Aileron"]["hinge_position"] - 0.05) / 100)**2)
    ...
    deltaC_h_alpha = 0.0025 * cl_alpha_cl_alpha_theory * cl_alpha_theory * B2 * K_alpha * 1 # 1 stand for the cosine of the quarter chord sweep of the wing
    ...
    c_doubleprime_h_delta = c_prime_h_delta + 2 * cl_delta_theory * (1 - cl_delta_cl_delta_theory) * (np.tan(PHI_doubleprime_te/2) - 0.137) #NOT SURE IF IT IS MAX or LOCAL t/c
    ...
    deltaC_h_delta = 0.0025 * cl_delta_cl_delta_theory * cl_delta_theory * B2 * K_alpha * 1 # 1 stand for the cosine of the quarter chord sweep of the wing
...
def StickForce(acd = aircraft_data, alpha=0.0, delta=0.0, h=3000, V=60.0):
    ...
    chord_a = chord(data=acd, location=(acd["Aileron"]["start"] + acd["Aileron"]["end"]) / 2 * (acd["Aero"]["b_Wing"] / 2))

    surface_a = 0.5 * acd["Aileron"]["hinge_position"] * (chord(data=acd, location=acd["Aileron"]["start"] * (acd["Aero"]["b_Wing"] / 2)) + chord(data=acd, location=acd["Aileron"]["end"] * (acd["Aero"]["b_Wing"] / 2))) * (acd["Aileron"]["end"] - acd["Aileron"]["start"]) * (acd["Aero"]["b_Wing"] / 2)

    HM = 0.5 * rho * V**2 * acd["Aileron"]["hinge_position"] * chord_a * surface_a * Ch

    d_aileron = 0.95 * 0.0063 / 0.1 * chord_a

    ratio = abs(HM) / d_aileron / 133.
