...
def InternalLoads(L, T, W, D, M, n, y_points, Cl_DATA, AoA, sweep):
    ...
    Vx = integrate.cumulative_trapezoid(np.flip(Dtot * b / (2 * n))[Dtot.shape[0]//2:], y_points[y_points.shape[0]//2:])[::-1]
    Vz = integrate.cumulative_trapezoid(np.flip((-L + W) * b / (2 * n))[W.shape[0]//2:], y_points[y_points.shape[0]//2:])[::-1]
    Vx = np.append(Vx, [0])
    Vz = np.append(Vz, [0])
