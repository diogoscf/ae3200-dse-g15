...
def plot_MTOW_vs_Payload(combined = True, show = None, save = None, ax = None):
    ...
    # single propeller
    MTOW_single_propeller = np.array([680.388, 997.903, 1065.942, 1202.02, 1202.02, 952.544, 1315.418, 1315.418, 1339.912]) * 9.81 # N
    Payload_single_propeller = np.array([180.529, 318.421, 326.1320, 332.9360, 324.3180, 145.6030, 494.4150, 383.2850, 335.6580]) * 9.81 # N

    # double propeller
    MTOW_double_propeller = np.array([3342.068582, 4399.845989, 3243.185446, 1814.36948, 2190.851147, 3810.175908, 3077.62423, 4501.904272, 4628.910136]) * 9.81 # N
    Payload_double_propeller = np.array([583.7733802, 668.5951534, 594.6595971, 332.4832072, 537.9605508, 861.825503, 544.310844, 725.747792, 889.0410452]) * 9.81 # N
    ...
    for MTOW in MTOW_:
        for Payload in Payload_:
            ...
            ax.plot(x_data, regressor_poly.predict(x_data_poly), color='green', markersize = 1, linewidth = 3, alpha = 0.7, label= '$Polynomial; R^2: 0.8501$')

            # show linear regression line
            regressor = LinearRegression()
            regressor.fit(Payload.reshape(-1, 1), MTOW)
            ax.plot(x_data, regressor.predict(x_data.reshape(-1, 1)), linestyle='dotted', markersize=1,
                         color='orange', linewidth = 3, alpha = 0.9, label= '$Linear; R^2: 0.8497$')

            # show exponential regression line
            regressor_exp = LinearRegression()
            regressor_exp.fit(Payload.reshape(-1, 1), np.log(MTOW))
            ax.plot(x_data, np.exp(regressor_exp.predict(x_data.reshape(-1, 1))), color='blue', alpha=0.7, linewidth = 2.5, label= '$Exponential; R^2: 0.772$')


            # show logarithmic regression line
            regressor_log = LinearRegression()
            regressor_log.fit(np.log(Payload).reshape(-1, 1), MTOW)
            ax.plot(x_data, regressor_log.predict(np.log(x_data).reshape(-1, 1)), color='red', alpha=0.7, linewidth = 2.5, label = '$Logarithmic; R^2: 0.75$')
...
def plot_MTOW_vs_OEW(combined = True, show = None, save = None, ax = None):
    ...
    # single propeller
    MTOW_single_propeller = np.array([680.388, 997.903, 1065.942, 1202.02, 1202.02, 952.544, 1315.418, 1315.418]) * 9.81 # N
    OEW_single_propeller = np.array([210.0132673, 304.8140726, 306.6284421, 338.379908, 346.9981631, 335.2047614, 406.8723559, 389.6358458]) * 9.81 # N

    # double propeller
    MTOW_double_propeller = np.array([3342.068582, 4399.845989, 3243.185446, 1814.36948, 2190.851147, 3810.175908, 3077.62423, 4501.904272, 4628.910136]) * 9.81 # N
    OEW_double_propeller = np.array([1098.147128, 1434.712666, 994.7280674, 573.3407557, 602.8242597, 1143.052772, 1122.187523, 1650.169042, 1782.164422]) * 9.81 # N
    ...
    for MTOW in MTOW_:
        for OEW in OEW_:
            ...
            if step == 1 or step == 4:
                ...
                ax.plot(x_data, regressor_poly.predict(x_data_poly), color='green', markersize = 1, linewidth = 3, alpha = 0.7, label = '$Polynomial; R^2:0.9814$')

                # show linear regression line
                regressor = LinearRegression()
                regressor.fit(MTOW.reshape(-1, 1), OEW)
                ax.plot(x_data, regressor.predict(x_data.reshape(-1, 1)), linestyle='dotted', markersize=1,
                         color='orange', linewidth = 3, alpha = 0.9, label = '$Linear; R^2: 0.9735$')

                # show exponential regression line
                regressor_exp = LinearRegression()
                regressor_exp.fit(MTOW.reshape(-1, 1), np.log(OEW))
                ax.plot(x_data, np.exp(regressor_exp.predict(x_data.reshape(-1, 1))), color='blue', alpha=0.7, linewidth = 2.5, label = '$Exponential; R^2: 0.9676$')


                # show logarithmic regression line
                regressor_log = LinearRegression()
                regressor_log.fit(np.log(MTOW).reshape(-1, 1), OEW)
                ax.plot(x_data, regressor_log.predict(np.log(x_data).reshape(-1, 1)), color='red', alpha=0.7, linewidth = 2.5, label = '$Logarithmic; R^2: 0.9039$')
...
def plot_MTOW_vs_WW(combined = True, show = None, save = None, ax = None):
    ...
    WW_single_propeller = np.array([97.97595192, 102.5118756, 102.965468, 106.594207, 106.594207, 107.9549841, 118.3876086]) * 9.81 # N

    # double propeller
    MTOW_double_propeller = np.array([3342.068582, 4399.845989, 3243.185446, 1814.36948, 2190.851147, 3810.175908, 3077.62423, 4501.904272, 4628.910136]) * 9.81 # N
    WW_double_propeller = np.array([303.9068879, 396.4397314, 297.5565947, 207.7453055, 205.4773436, 390.0894382, 289.3919321, 395.986139, 454.0459624]) * 9.81 # N
    ...
    for MTOW in MTOW_:
        for WW in WW_:
            ...
            if step == 1 or step == 4:
                ...
                ax.plot(x_data, regressor_poly.predict(x_data_poly), color='green', markersize = 1, linewidth = 3, alpha = 0.7, label = '$Polynomial; R^2:0.981$')

                # show linear regression line
                regressor = LinearRegression()
                regressor.fit(MTOW.reshape(-1, 1), WW)
                ax.plot(x_data, regressor.predict(x_data.reshape(-1, 1)), linestyle='dotted', markersize=1,
                         color='orange', linewidth = 3, alpha = 0.9, label = '$Linear; R^2: 0.9809$')

                # show exponential regression line
                regressor_exp = LinearRegression()
                regressor_exp.fit(MTOW.reshape(-1, 1), np.log(WW))
                ax.plot(x_data, np.exp(regressor_exp.predict(x_data.reshape(-1, 1))), color='blue', alpha=0.7, linewidth = 2.5, label = '$Exponential; R^2: 0.9489$')


                # show logarithmic regression line
                regressor_log = LinearRegression()
                regressor_log.fit(np.log(MTOW).reshape(-1, 1), WW)
                ax.plot(x_data, regressor_log.predict(np.log(x_data).reshape(-1, 1)), color='red', alpha=0.7, linewidth = 2.5, label = '$Logarithmic; R^2:0.9307$')
