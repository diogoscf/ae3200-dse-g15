...
def plot_MTOW_vs_Payload(combined = True, show = None, save = None, ax = None):
    ...
    ax.plot(x_data, regressor_poly.predict(x_data_poly), color='green', markersize = 1, linewidth = 3, alpha = 0.7, label= '$Polynomial; R^2: 0.9967$')
    ...
    ax.plot(x_data, regressor.predict(x_data.reshape(-1, 1)), linestyle='dotted', markersize=1,
                 color='orange', linewidth = 3, alpha = 0.9, label= '$Linear; R^2: 0.978$')

    # show exponential regression line
    regressor_exp = LinearRegression()
    regressor_exp.fit(Payload.reshape(-1, 1), np.log(MTOW))
    ax.plot(x_data, np.exp(regressor_exp.predict(x_data.reshape(-1, 1))), color='blue', alpha=0.7, linewidth = 2.5, label= '$Exponential; R^2: 0.9637$')


    # show logarithmic regression line
    regressor_log = LinearRegression()
    regressor_log.fit(np.log(Payload).reshape(-1, 1), MTOW)
    ax.plot(x_data, regressor_log.predict(np.log(x_data).reshape(-1, 1)), color='red', alpha=0.7, linewidth = 2.5, label = '$Logarithmic; R^2: 0.8502$')
...
def plot_MTOW_vs_OEW(combined = True, show = None, save = None, ax = None):
    ...
    ax.plot(x_data, regressor_poly.predict(x_data_poly), color='green', markersize = 1, linewidth = 3, alpha = 0.7, label = '$Polynomial; R^2:0.9917$')

    # show linear regression line
    regressor = LinearRegression()
    regressor.fit(MTOW.reshape(-1, 1), OEW)
    ax.plot(x_data, regressor.predict(x_data.reshape(-1, 1)), linestyle='dotted', markersize=1,
             color='orange', linewidth = 3, alpha = 0.9, label = '$Linear; R^2: 0.9895$')

    # show exponential regression line
    regressor_exp = LinearRegression()
    regressor_exp.fit(MTOW.reshape(-1, 1), np.log(OEW))
    ax.plot(x_data, np.exp(regressor_exp.predict(x_data.reshape(-1, 1))), color='blue', alpha=0.7, linewidth = 2.5, label = '$Exponential; R^2: 0.8524$')


    # show logarithmic regression line
    regressor_log = LinearRegression()
    regressor_log.fit(np.log(MTOW).reshape(-1, 1), OEW)
    ax.plot(x_data, regressor_log.predict(np.log(x_data).reshape(-1, 1)), color='red', alpha=0.7, linewidth = 2.5, label = '$Logarithmic; R^2: 0.8969$')
