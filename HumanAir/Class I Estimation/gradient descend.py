...
def objective_function(params):
    ...
    for step in range(len(bat)):
        working_dict['P_req_cruise_W'] = working_dict['P_cruise/P_TO'] * np.exp(coeff_exp[1]) * np.exp(
            coeff_exp[0] * bat[step])
        working_dict['E_bat_Wh'] = working_dict['P_req_cruise_W'] * working_dict['endurance'] / working_dict[
            'P_cruise/P_TO'] * bat[step]
